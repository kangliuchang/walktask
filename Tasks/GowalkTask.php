...
class GowalkTask implements TaskInterface
{
...
//        }
    ...
    public function rewards()
    {
        try {
            ...
            //检查有没有完成
            if (!$this->check())
                throw new \Exception('您还未完成一次2公里健走里程记录',50000);
            //检查助力次数
    ...
    public function check()
    {
        try {
            //查有没有路线
            $recode = M('individual_work')->where("author_id = {$this->authorId} AND create_at > {$this->task['start_at']} AND create_at < {$this->task['end_at']} AND kilometre > 2")->find();
            if ($recode){
