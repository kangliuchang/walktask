...
class teamPrivateActivityTask implements TaskInterface
{
...
//        }
    ...
    public function rewards()
    {
        try {
            ...
            //检查有没有完成
            if (!$this->check())
                throw new \Exception('您还未完成任一专属团队路线任务',50000);
            //检查助力次数
    ...
    public function check()
    {
        try {
            //查有没有加入过公开的团队任务
            $sql = "SELECT a.id FROM svideo_individual_work AS a JOIN svideo_activity AS b ON a.act_id = b.act_id AND b.status = 1 WHERE a.author_id = {$this->authorId} AND a.create_at > {$this->task['start_at']} AND a.create_at < {$this->task['end_at']} AND a.is_complete = 1";
